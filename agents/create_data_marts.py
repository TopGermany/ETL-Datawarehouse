...
def create_marts(
    ...
    project_id = project_id or os.getenv("GCP_PROJECT_ID", "haian-dwh-project")
    dataset_id = dataset_id or os.getenv("BQ_DATASET_ID", "haian_dwh")
    ...
    # 4. View: mart_haian_vs_market_price
    sql_comparison = f"""
    ...
    # ── Dry-run: trả kết quả mà không thực thi BigQuery ──────────────────
    if dry_run:
        return {
    ...
    return {
        "tool": "create_data_marts",
        "status": "ok",
        "summary": "Created 4 BigQuery views",
        "data": {
            "project_id": project_id,
            "dataset_id": dataset_id,
            "view_count": 4,
